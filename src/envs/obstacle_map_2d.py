...
class ObstacleMap:
    ...
    def compute_cost(self, x: torch.Tensor, inital_state=None) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        detect_range_occ = torch.tensor(self._detect_range).to(self._device, self._dtype) / self._cell_size
        detect_range_occ = torch.round(detect_range_occ).long().to(self._device)
        inital_x_occ = (inital_state[:2] / self._cell_size) + self._torch_cell_map_origin
        ...
        return collisions, range_render
